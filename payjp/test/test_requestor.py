...
class APIRequestorRequestTests(PayjpUnitTestCase):
    ...
    def mock_response(self, return_body, return_code, requestor=None):
        ...
        self.http_client.request = Mock(return_value=(return_body, return_code))

    def check_call(
        self, meth, abs_url=None, headers=None, post_data=None, requestor=None
    ):
        if not abs_url:
    ...
    def test_url_construction(self):
        CASES = (
            ...
            ("https://api.pay.jp?foo=bar&foo=bar", "?foo=bar", {"foo": "bar"}),
        )
    ...
    def test_methods_with_params_and_response(self):
        for meth in VALID_API_METHODS:
            ...
            encoded = (
                "adict%5Bfrobble%5D=bits&adatetime=1356994800&"
            ...
            if meth == "post":
                self.check_call(meth, post_data=QueryMatcher(parse_qsl(encoded)))
            else:
                abs_url = "https://api.pay.jp%s?%s" % (self.valid_path, encoded)
    ...
    def test_uses_instance_key(self):
        ...
        self.check_call(
            "get",
            headers=APIHeaderMatcher(key, request_method="get"),
            requestor=requestor,
        )
    ...
    def test_uses_instance_account(self):
        account = "acct_foo"
        requestor = payjp.api_requestor.APIRequestor(
            account=account, client=self.http_client
        )
        ...
        self.check_call(
            ...
            requestor=requestor,
            headers=APIHeaderMatcher(
                extra={"Payjp-Account": account}, request_method="get"
            ),
        )
    ...
    def test_fails_without_api_key(self):
        ...
        self.assertRaises(
            payjp.error.AuthenticationError,
    ...
    def test_not_found(self):
        ...
        self.assertRaises(
            payjp.error.InvalidRequestError,
            self.requestor.request,
            "get",
            self.valid_path,
            {},
    ...
    def test_card_error(self):
        ...
        self.assertRaises(
            payjp.error.CardError, self.requestor.request, "get", self.valid_path, {}
        )
    ...
    def test_too_many_request_error(self):
        ...
        self.assertRaises(
            payjp.error.APIError, self.requestor.request, "get", self.valid_path, {}
        )
    ...
    def test_invalid_method(self):
        self.assertRaises(
            payjp.error.APIConnectionError, self.requestor.request, "foo", "bar"
        )
...
class APIRequestorRetryTest(PayjpUnitTestCase):
    def setUp(self):
        ...
        def return_value_generator():
            for status in self.return_values:
                yield (
                    '{{"error": {{"status": {status}, "message": "test"}}}}'.format(
                        status=status
                    ),
                    status,
                    "sk_live_aaa",
        ...
        self.request_raw_patch = patch(
            "payjp.api_requestor.APIRequestor.request_raw", request_raw
        )
    ...
    def test_full_retry(self):
        ...
        self.return_values = [
            ...
            429,
            200,
        ]  # first try + 2 retries + unexpected 200
        with self.request_raw_patch:
            with self.assertRaises(payjp.error.APIError) as error:
                self.requestor.request("get", "/test", {})
